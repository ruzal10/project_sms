...
if (isset($_POST['submit'])) {
    $student = $_POST['name'];
    $pass = $_POST['password'];
    $qry = "SELECT * FROM student WHERE username='$student' AND password='$pass' AND status = 'verified'";
    if ($res = mysqli_query($con, $qry)) {
        if (mysqli_num_rows($res) > 0) {
            $_SESSION['student'] = $student;
            echo '<script>window.location.href="studentdashboard.php";</script>';
        } else {
            echo '<script>alert("Invalid Username or Password or You are not registerd by admin yet.")</script>';
        }
    }
}
$con->close();
